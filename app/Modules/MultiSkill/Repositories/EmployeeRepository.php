...
use Modules\MultiSkill\Models\EmployeeModel;
use Config\Database;
...
class EmployeeRepository
{
    ...
    public function getFilteredEmployees(array $filters = [], int $limit = 50, int $offset = 0)
    {
        ...
        if (empty($filters['sort_by'])) {
            ...
            $builder->orderBy("CASE WHEN section = 'Cutting' THEN 1 WHEN section = 'Sewing' THEN 2 WHEN section = 'Assembly' THEN 3 ELSE 4 END", 'ASC');
    ...
    public function getAnalyticsMetrics()
    {
        ...
        $totalActive = $this->db->table('employees')->where('status', 'Active')->countAllResults();
        $totalAll = $this->db->table('employees')->countAllResults();

        // 2. Total Multi Skill employees (Active, skills >= 3)
        $totalMultiskill = $this->db->table('employees')
            ->where('status', 'Active')
        ...
        $bySection = $this->db->table('employees')
        ...
        // 5. Employees by Line
        $byLine = $this->db->table('employees')
        ...
        $topEmployees = $this->db->table('employees')
        ...
        $skillQuery = "
            SELECT skill, COUNT(*) as count FROM (
                SELECT skill_1 as skill FROM employees WHERE skill_1 IS NOT NULL AND skill_1 != ''
                UNION ALL
                SELECT skill_2 as skill FROM employees WHERE skill_2 IS NOT NULL AND skill_2 != ''
                UNION ALL
                SELECT skill_3 as skill FROM employees WHERE skill_3 IS NOT NULL AND skill_3 != ''
                UNION ALL
                SELECT skill_4 as skill FROM employees WHERE skill_4 IS NOT NULL AND skill_4 != ''
                UNION ALL
                SELECT skill_5 as skill FROM employees WHERE skill_5 IS NOT NULL AND skill_5 != ''
                UNION ALL
                SELECT skill_6 as skill FROM employees WHERE skill_6 IS NOT NULL AND skill_6 != ''
                UNION ALL
                SELECT skill_7 as skill FROM employees WHERE skill_7 IS NOT NULL AND skill_7 != ''
                UNION ALL
                SELECT skill_8 as skill FROM employees WHERE skill_8 IS NOT NULL AND skill_8 != ''
                UNION ALL
                SELECT skill_9 as skill FROM employees WHERE skill_9 IS NOT NULL AND skill_9 != ''
                UNION ALL
                SELECT skill_10 as skill FROM employees WHERE skill_10 IS NOT NULL AND skill_10 != ''
            ) AS all_skills
            GROUP BY skill
            ORDER BY count DESC
            LIMIT 10
        ...
        $recentActivities = $this->db->table('employees')
